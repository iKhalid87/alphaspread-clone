  ...
  return (
    <div className="relative mb-4">
      <div className="flex gap-2">
        <input
          className="bg-gray-800 p-2 rounded w-full border border-gray-600"
          placeholder="Enter stock symbol (e.g. AAPL)"
          value={input}
          onChange={(e) => setInput(e.target.value)}
        />
        <button className="bg-blue-600 hover:bg-blue-500 px-4 py-2 rounded" onClick={() => handleSearch()}>
          Search
        </button>
      </div>
      {suggestions.length > 0 && (
        <ul className="absolute bg-gray-700 w-full rounded mt-1 z-10 border border-gray-600">
          {suggestions.map(sym => (
            <li
              key={sym}
